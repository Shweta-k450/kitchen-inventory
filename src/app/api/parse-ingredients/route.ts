...
export async function POST(request: Request) {
  ...
  const prompt =
    ...
    'For each ingredient reply with one JSON object: {"text": the ingredient line as written, "name": a short lowercase singular core ingredient name with quantities/units/prep words removed (e.g. "2 cups chopped yellow onion" -> "onion"), "quantity": the amount or size as written, or null if none, "amount": the numeric quantity as a number (e.g. 2, 0.5, 1.5), or null if the line has no number, "unit": one of exactly these — g, kg, mL, L, tsp, tbsp, cup, oz, lb, clove, slice, pinch, pack, count — best matching how the ingredient is measured; use "count" for whole items measured by number (2 eggs, 1 onion) and null when there is no amount, "category": one of exactly these ids — ' +
    catIds +
